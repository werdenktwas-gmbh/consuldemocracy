class Poll::WebVote
  # TODO: handle errors, validations, ...
  ...
  def update(params)
    user.with_lock do
      questions.each do |question|
        ...
        next unless params[question.id.to_s] # TODO: hidden field instead?

        option_ids = params[question.id.to_s][:option_id]

        answers = Array(option_ids).map do |option_id|
          text_answer = nil
          if(params[question.id.to_s][option_id] && params[question.id.to_s][option_id]['text_answer']) 
            text_answer = params[question.id.to_s][option_id]['text_answer']
          end
